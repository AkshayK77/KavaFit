...
const ManualWorkoutLogger = React.memo(function ManualWorkoutLogger({ onClose, onSaved, editSessionId = null }: {
  ...
  async function loadEditSession() {
    setLoadingEdit(true)
    try {
      type SessRow = { id: string; name: string | null; date: string; duration_minutes: number | null }
      ...
      if (sets?.length) {
        ...
        sets.forEach(s => {
          if (!grouped[s.exercise_id]) {
            const exData = exMap[s.exercise_id]
            grouped[s.exercise_id] = {
              exercise: exData || { id: s.exercise_id, name: 'Unknown', muscle_groups: [] },
              sets: []
            }
  ...
  async function resolveExerciseId(ex: ExerciseItem): Promise<string> {
    ...
    const { data: created, error } = await (supabase.from('exercises') as any)
      .insert({ name: ex.name, muscle_groups: ex.muscle_groups || [] }).select('id').single()
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      if (oldSetsTyped?.length) {
        const oldExIds = [...new Set(oldSetsTyped.map(s => s.exercise_id))]
        ...
        oldSetsForVolume = oldSetsTyped.map(s => ({ muscle_groups: exMgMap[s.exercise_id] || [] }))
      }
