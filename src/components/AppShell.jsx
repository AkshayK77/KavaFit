...
export default function AppShell({ children }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      const startTime = stored.startTime
      if (!startTime) { setActiveTimer(null); return }

      const now = Date.now()
      const pausedTime = stored.pausedTime || 0
      const isPaused = !!stored.isPaused
      const pauseStart = stored.pauseStart || null
      ...
      setActiveTimer({ sessionName: stored.activeSession?.name || 'Active session', elapsed, isPaused })
    }, 1000)
...
function GridIcon({ size = 22, active }) {
  return (
...
function DumbbellIcon({ size = 22, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function BodyIcon({ size = 22, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function ChartIcon({ size = 22, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function NutritionIcon({ size = 22, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
function ChatNavIcon({ size = 22, active }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
