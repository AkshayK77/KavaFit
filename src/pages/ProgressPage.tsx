...
function formatWorkoutName(sessionName: string | null | undefined, exercises: Array<{ exercise?: { muscle_groups?: string[] } }>) {
...
function LineGraph({ points, color = '#C8F55A', height = 160, unit = 'kg' }: { points: Array<{x: number; y: number}>; color?: string; height?: number; unit?: string }) {
...
export default function ProgressPage() {
  ...
  // Section B — PRs
  const [prs, setPrs] = useState<Array<{name: string; weight: number; reps?: number; date?: string; isNew: boolean}>>([])
  ...
  async function loadPRs() {
    ...
    const list = Object.values(prMap)
      .filter(p => p.name)
      .map(p => ({ ...p, isNew: new Date(p.date ?? '').getTime() > sevenAgo.getTime() }))
      .sort((a, b) => (b.isNew ? 1 : 0) - (a.isNew ? 1 : 0) || b.weight - a.weight)
    setPrs(list as typeof prs)
  }
  ...
  async function saveMeasurements() {
    ...
    fields.forEach(f => { if ((measForm as Record<string, string>)[f]) row[f] = parseFloat((measForm as Record<string, string>)[f]) })
    await (supabase.from('measurements') as any).insert(row)
    setSavingMeas(false)
  ...
  async function loadMonthSessionDates(year: number, month: number) {
    ...
    const rows = data as Array<{ date: string }> | null
    setSessionDates(new Set((rows || []).map(s => s.date)))
  }
  ...
  async function handleDateClick(dateStr: string) {
    ...
    const { data: sessData } = await supabase
      ...
      .eq('user_id', user!.id)
      .eq('date', dateStr)
    type SessRow = { id: string; date: string; name: string | null; notes: string | null; duration_minutes: number | null; plan_day_id: string | null; plan_days: { id: string; day_name: string; exercise_ids: unknown } | null }
    ...
    const plannedIds = sessions.flatMap(sess => {
      const raw = Array.isArray(sess.plan_days?.exercise_ids) ? sess.plan_days!.exercise_ids as Array<Record<string, unknown>> : []
      return raw.map(e => (typeof e === 'object' && e !== null ? (e as { exerciseId?: string }).exerciseId : e) as string).filter(Boolean)
    })
    ...
    const exIds = [...new Set([...(sets || []).map(s => s.exercise_id).filter(Boolean) as string[], ...plannedIds])]
    let exMap: Record<string, { id: string; name: string; muscle_groups?: string[] }> = {}
    if (exIds.length) {
      const { data: exsData } = await supabase.from('exercises').select('id,name,muscle_groups').in('id', exIds)
      const exs = exsData as Array<{ id: string; name: string; muscle_groups?: string[] }> | null
      exs?.forEach(e => { exMap[e.id] = e })
    }
    const enriched = sessions.map(sess => {
      const ss = (sets || []).filter(s => s.session_id === sess.id)
      const byEx: Record<string, Array<Record<string, unknown>>> = {}
      ss.forEach(s => { if (!byEx[String(s.exercise_id)]) byEx[String(s.exercise_id)] = []; byEx[String(s.exercise_id)].push(s as Record<string, unknown>) })
      const rawPlanIds = Array.isArray(sess.plan_days?.exercise_ids) ? sess.plan_days!.exercise_ids as Array<Record<string, unknown>> : []
      const planExerciseIds = rawPlanIds.map(e => (typeof e === 'object' && e !== null ? (e as { exerciseId?: string }).exerciseId : e) as string).filter(Boolean)
      ...
      const workoutName = formatWorkoutName(sess.name || (parsedNotes.sessionName as string | undefined) || sess.plan_days?.day_name, exercises)
  ...
  async function doSearch(query: string) {
    ...
    const results = await Promise.all(exercises.map(async ex => {
      const { data: setsData } = await supabase
        ...
        .limit(30)
      type SetHistRow = { weight_kg: number | null; reps: number | null; created_at: string; sessions: { date: string } | null }
      const sets = setsData as SetHistRow[] | null
      const history = (sets || []).map(s => ({ date: s.sessions?.date, weight: parseFloat(String(s.weight_kg ?? 0)) || 0, reps: s.reps ?? undefined, x: new Date(s.created_at).getTime(), y: parseFloat(String(s.weight_kg ?? 0)) || 0 }))
  ...
  const comparePhotos = selectedPhotoIds.length === 2
    ? selectedPhotoIds.map(id => photos.find(p => p.id === id)).filter((p): p is typeof photos[number] => !!p)
    : null
    ...
    <div style={{ ...s.page, padding: isMobile ? '16px 16px 40px' : '28px 28px 60px' }}>
              ...
              return (
                <div key={k} style={s.formField}>
                  ...
                  <input
                    ...
                    placeholder={i === 0 ? '' : '0.0'}
                    value={measForm[k as keyof typeof measForm]}
      ...
      <div style={s.section}>
        ...
        {selectedDate && !detailLoading && selectedSessions.length > 0 && (selectedSessions as Array<{
          id: string; name: string | null; date: string; duration_minutes: number | null
          exercises: Array<{ exercise: { id?: string; name?: string } | null; sets: Array<Record<string, unknown>>; completed: number }>
          workoutName?: string
        }>).map(sess => (
          <div key={sess.id} style={s.detailBlock}>
            <div style={s.detailTitle}>{formatWorkoutName(sess.name, sess.exercises as Parameters<typeof formatWorkoutName>[1])}</div>
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <span style={{ fontSize: '12px', color: 'var(--muted)' }}>
                {sess.exercises.map((e: { exercise: { name?: string } | null }) => e.exercise?.name || 'Unknown').join(', ') || '—'}
              </span>
            </div>
            <div style={{ marginBottom: '12px' }}>
              <span style={{ ...s.sectionLabel, marginRight: '8px' }}>Completed</span>
              <span style={{ fontSize: '12px', color: 'var(--muted)' }}>
                {sess.exercises.filter((e: { completed: number }) => e.completed > 0).map((e: { exercise: { name?: string } | null }) => e.exercise?.name || 'Unknown').join(', ') || '—'}
              </span>
            </div>
            {sess.exercises.map(({ exercise, sets }: { exercise: { id?: string; name?: string } | null; sets: Array<Record<string, unknown>> }) => (
              <div key={exercise?.id ?? Math.random()} style={s.exBlock}>
                ...
                {sets.map((set: Record<string, unknown>, i: number) => (
                  <div key={i} style={s.setRowD}>
                    <div style={{ color: 'var(--muted)' }}>{set.set_number as number}</div>
                    <div>{set.weight_kg != null ? `${set.weight_kg}` : '—'}kg</div>
                    <div>{set.reps != null ? `${set.reps}` : '—'}</div>
                    <div style={{ color: 'var(--dim)' }}>{set.rpe != null ? `${set.rpe}` : '—'}</div>
                  </div>
                ))}
              </div>
            ))}
